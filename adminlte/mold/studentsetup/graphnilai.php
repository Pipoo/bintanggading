...
<?
    ...
    foreach($graph as $num=>$result)
    {
        ...
        $total += $result->nilai;
        $jml++;
        if($small>$result->nilai) $small = $result->nilai;
        if($big<$result->nilai) $big = $result->nilai;
    }
...
?>
<style type="text/css">
    .nilainya{ font-size: 13em; text-align: center; width: 100%;}
    .tglnilai,.isinilai{ float:left; width: 50%;}
